 ...
 * 
 * Which this test does not do, meaning that if you modiify 
...
// The callback/function to be ran in a threading context
...
// The callback/function to be ran in a C thread 
int c_threads_callback(void* vdata) {
    CThreadData *request = vdata;

    return tfhe_threading_context_run(
        request->context,
        do_work_in_tfhe_threading_context,
        (void*)request->data
    );
}

// Main part of the work, spaws threads and puts calculations on them
void do_thing(const ClientKey *client_key, const ServerKey* server_key) {
    ...
    for (size_t i = 0; i < NUM_C_THREADS; ++i) {
        status = fhe_uint64_try_encrypt_with_client_key_u64(clear_inputs[i][0], client_key, &data[i].inputs[0]);
        assert(status == 0);
        status = fhe_uint64_try_encrypt_with_client_key_u64(clear_inputs[i][1], client_key, &data[i].inputs[1]);
        assert(status == 0);
